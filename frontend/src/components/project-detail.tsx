...
const styles = {
  chat: { surface: "bg-cyan-100", accent: "bg-cyan-400", text: "text-cyan-700", button: "bg-cyan-500 hover:bg-cyan-600", metric: "18ms", label: "Message delivery" },
  api: { surface: "bg-lime-100", accent: "bg-lime-400", text: "text-lime-700", button: "bg-lime-500 hover:bg-lime-600", metric: "JWT", label: "Protected access" },
  crawler: { surface: "bg-violet-100", accent: "bg-violet-400", text: "text-violet-700", button: "bg-violet-500 hover:bg-violet-600", metric: "50+", label: "Concurrent workers" }
};
      ...
      <div className="mx-auto max-w-7xl">
          ...
          <div className="relative grid lg:grid-cols-[0.9fr_1.1fr]">
            <div className="flex min-w-0 flex-col justify-center p-7 sm:p-12 lg:p-14">
              <div className="flex items-center gap-3">
                ...
                <div><p className={`text-xs font-black uppercase tracking-[0.18em] ${style.text}`}>{project.eyebrow}</p><p className="mt-1 flex items-center gap-1.5 text-[10px] font-black uppercase text-emerald-600"><Radio className="h-3 w-3 animate-pulse" /> Production deployment live</p></div>
              </div>
            ...
            <div className={`relative min-w-0 overflow-hidden p-5 sm:p-8 lg:p-10 ${style.surface}`}>
              ...
              <div className="relative flex h-full min-h-[560px] flex-col justify-center">
                <div className="mb-4 flex items-end justify-between gap-4"><div><p className={`text-[10px] font-black uppercase tracking-[0.15em] ${style.text}`}>Live product view</p><p className="mt-1 text-xs font-bold text-slate-500">Interactive system preview</p></div><motion.div className="text-right" animate={{ y: [0, -4, 0] }} transition={{ duration: 3, repeat: Infinity }}><p className="text-3xl font-black text-slate-950">{style.metric}</p><p className="text-[9px] font-black uppercase tracking-[0.1em] text-slate-500">{style.label}</p></motion.div></div>
                ...
                <div className="mt-4 grid grid-cols-3 gap-2">{[[project.languages[0], "Core"], [String(project.features.length), "Features"], ["Online", "Status"]].map(([value, label], index) => <motion.div key={label} className="rounded-md bg-white/75 p-3 backdrop-blur" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.55 + index * 0.1 }}><p className="text-sm font-black text-slate-950">{value}</p><p className="mt-1 text-[9px] font-bold uppercase tracking-[0.1em] text-slate-500">{label}</p></motion.div>)}</div>
              </div>
            </div>
          </div>
        </motion.section>
        ...
        <motion.section className="mt-6 overflow-hidden rounded-lg border border-slate-200 bg-white p-7 shadow-sm sm:p-10" initial={{ opacity: 0, y: 35 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.7 }}>
          ...
          <div className="relative mt-10 grid gap-4 md:grid-cols-4">
            ...
            {project.technologies.slice(0, 4).map((tech, index) => <motion.div key={tech} className="relative rounded-lg border border-slate-200 bg-slate-50 p-5" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.12 }} whileHover={{ y: -6, backgroundColor: "#ffffff" }}><span className={`relative grid h-12 w-12 place-items-center rounded-full ${style.accent} text-sm font-black text-slate-950 shadow-lg`}>{String(index + 1).padStart(2, "0")}</span><p className="mt-6 text-sm font-black text-slate-950">{tech}</p><p className="mt-2 text-xs leading-5 text-slate-500">{index === 0 ? "Receives and validates input." : index === 3 ? "Returns a structured result." : "Processes the request safely."}</p></motion.div>)}
          </div>
        </motion.section>
