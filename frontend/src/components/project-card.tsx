    ...
    >
      <div className="grid lg:grid-cols-[0.9fr_1.1fr]">
        <div className={`flex min-w-0 flex-col justify-between p-7 sm:p-10 lg:p-12 ${reversed ? "lg:order-2" : ""}`}>
          ...
          <div className="mt-9">
            <div className="flex flex-wrap gap-2">{project.technologies.slice(0, 6).map((tech) => <motion.span key={tech} className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-[10px] font-black text-slate-500" whileHover={{ y: -3, borderColor: "#2563eb", color: "#2563eb" }}>{tech}</motion.span>)}</div>
            <div className="mt-7 flex flex-wrap gap-3">
              ...
              <Link href={project.demo} className="group flex h-12 items-center gap-2 rounded-md border border-slate-200 px-4 text-xs font-black uppercase text-slate-700 transition hover:-translate-y-1 hover:border-slate-950"><Play className="h-4 w-4 transition group-hover:scale-110" /> Live demo</Link>
