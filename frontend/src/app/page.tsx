  ...
  return (
    <main>
      ...
      <section className="border-t border-slate-200/80 bg-white px-3 py-24 sm:px-6">
        <AnimatedSection className="mx-auto grid max-w-7xl overflow-hidden rounded-lg border border-slate-200 bg-white shadow-[0_30px_100px_rgba(15,23,42,.10)] lg:grid-cols-[0.9fr_1.1fr]">
          <div className="relative overflow-hidden border-b border-blue-500 bg-blue-600 p-8 text-white sm:p-12 lg:border-b-0 lg:border-r"><div className="absolute inset-0 bg-[linear-gradient(rgba(255,255,255,.08)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,.08)_1px,transparent_1px)] bg-[size:40px_40px]" /><div className="relative"><p className="text-xs font-black uppercase tracking-[0.18em] text-blue-100">About the engineer</p><motion.h2 className="mt-5 text-6xl font-black" initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>{owner.name}</motion.h2><p className="mt-3 text-blue-100">{owner.role}</p><div className="mt-12 flex items-start gap-3 border-t border-white/20 pt-6"><GraduationCap className="h-5 w-5 shrink-0 text-cyan-200" /><p className="text-sm leading-6 text-blue-50">{owner.university}<br />Student ID: {owner.studentId}</p></div><Link href="/profile" className="group mt-8 inline-flex items-center gap-2 rounded-md bg-white px-5 py-3 text-xs font-black uppercase text-blue-700 transition hover:bg-lime-300">Meet Rehan <ArrowRight className="h-4 w-4 transition group-hover:translate-x-1" /></Link></div></div>
